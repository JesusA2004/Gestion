            ...
            <div class="max-w-7xl mx-auto space-y-6">
                ...
                <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                    <div>
                        ...
                        <p class="mt-2 text-sm md:text-base text-gray-500 max-w-xl">
                            Gestión de departamentos: alta, edición y eliminación en una sola pantalla.
                        </p>
                    </div>
                ...
                <div class="rounded-2xl bg-white/90 backdrop-blur shadow-sm border border-slate-200 px-4 py-4 sm:px-6 sm:py-5">
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 md:items-end">
                        {{-- Buscador texto (2/3 ancho en desktop) --}}
                        <div class="md:col-span-2">
                            <label for="departamento-search-text" class="block text-[11px] md:text-xs font-semibold text-slate-500 uppercase tracking-wide">
                                Buscar por nombre o dirección
                            </label>
                            <div class="mt-1 relative">
                                <input
                                    id="departamento-search-text"
                                    type="text"
                    ...
                    <table class="min-w-full divide-y divide-slate-200 text-[13px] md:text-sm">
                        ...
                        >
                            @forelse($departamentos as $departamento)
                                <tr
                                    class="departamento-row hover:bg-indigo-50/60 transition-colors"
                                    data-departamento-row
                                    data-nombre="{{ strtolower($departamento->nombre) }}"
                                    data-direccion="{{ strtolower($departamento->direccion ?? '') }}"
                                        ...
                                        <div class="space-y-1.5">
                                            <div>
                                                <span class="block font-semibold text-slate-800">
                                                    ...
                                                    a las
                                                    {{ optional($departamento->created_at)->format('H:i') }} hrs
                                                </span>
                                            </div>
                                            <div class="text-slate-600">
                                                <span class="block font-semibold text-slate-800">
                                                    Modificado el
                                                    {{ optional($departamento->updated_at)->format('d-m-y') }}
                                                    a las
                                                    {{ optional($departamento->updated_at)->format('H:i') }} hrs
                                                </span>
                                            </div>
                                        </div>
                                    </td>
